...
你可以在一个清晰的控制面板中管理比分、计时、节次、进攻时间、球权、犯规、牌、换人、球员名单、足球补时、棋钟、冰球罚时计时器和辩论准备时间。应用也可以通过外接显示器、AirPlay 或一个或多个已配对的远程显示设备展示全屏公共记分牌，同时把操作控制保留在操作员设备上。

选择 **本机显示 → 全屏显示** 可展示全屏记分牌，或选择 **合并视图（测试版）**，在同一窗口中操作比赛并查看记分牌。组合视图在横屏和竖屏时均将记分牌置于紧凑、可滚动的控制面板上方，并根据 iPhone、iPad 和 Mac 的窗口尺寸调整控制面板的列数。
...
- 实时比分、节次和比赛计时控制
- 足球和自定义倒计时模式支持补时控制，并在公共显示上显示 +N 分钟
